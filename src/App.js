...
export default class App extends React.PureComponent {
  ...
  getTabsData() {
    Promise.all([this.getAllTabs(), this.getCurrentTab(), Helper.waitFor(300)])
  ...
  onFilterTab = (key) => {
    const filteredTabs = this.allTabs.filter(
      (tab) => tab.title.indexOf(key) > -1 || tab.url.indexOf(key) > -1
    );
