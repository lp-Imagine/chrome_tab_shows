  ...
  renderOK() {
    ...
    return (
      <React.Fragment>
        ...
        <Collapse expandIconPosition={"right"} defaultActiveKey={DEFAULT_OPEN}>
          {data.map((item, index) => {
              ...
              <Collapse.Panel
                ...
                header={
                  <Row className={"window-header-container"}>
                    ...
                    <Typography.Text
                      type={"secondary"}
                      className={"window-header-subTitle-text"}
                    >
                      （
                      <span className={"window-header-tabnums"}>
                        {item.tabsCount}
                      </span>
                      tabs）
                    </Typography.Text>
...
class WindowItem extends React.PureComponent {
  renderItem = (item) => {
    ...
    return (
      <DomainItem {...item} isCurWindow={isCurWindow} onRemove={onRemove} />
    );
  };
...
class DomainItem extends React.PureComponent {
  ...
  renderSingleTab(item, index) {
    ...
    return (
      ...
      >
        <Avatar size={16} shape={"shape"} src={item.favIconUrl} />
        <Typography.Text
          ellipsis
          strong={isSelected}
    ...
    return (
      ...
      >
        <Collapse.Panel
          header={
            <Row className={"window-header-container"}>
              <span className={"domain-header-title-text"}>
                <Avatar size={16} shape={"shape"} src={tabs[0].favIconUrl} />{" "}
                {domain}
              </span>
              <Button
                type="primary"
